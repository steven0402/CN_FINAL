...
using namespace std;
char *login_file_path = "login_config.txt";
...
Server::Server(int portnum) {
	printf("constructor called with portnum %d\n", portnum);
	port = portnum;
	
}

void clean_msg(char *receivemsg, int len) {
	for (int i = 0; i < len; i++) receivemsg[i] = '\0';
}
...
string get_string(char *message, int read_count){
	string temp;
...
string read_file(char *message, int read_count){
	string temp;
	while(message[read_count])
	{
		if (read_count == 1000) return temp;
		temp += message[read_count];
		read_count++;
...
void load_username(){
	FILE *fd = fopen(login_file_path, "r+");
	if (fd == NULL)
...
void handle_client(int clientsockfd, string username){
...
void* start_connect(void* sockfd)
{
	...
	while(login == 0){
		...
		else printf("recv error on fd %d\n", clientsockfd);
	}
...
int main(int argc, char *argv[])
{
	...
	while(clientsockfd = accept(sockfd, (struct sockaddr *)&their_addr, &addr_size)){
		//int flag = 1;
		//setsockopt(clientsockfd, IPPROTO_TCP, TCP_NODELAY, (char *)&flag, sizeof(int));
