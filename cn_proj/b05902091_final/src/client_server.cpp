...
int connect_to_server(int sockfd, string& IP, string& port) {
	...
	// printf("IP: %s, port: %s\n", IP_addr, portnum);
	struct addrinfo hints; memset(&hints, 0, sizeof(hints)); // make sure the struct is empty
	hints.ai_family = AF_UNSPEC;     // don't care IPv4 or IPv6
	hints.ai_socktype = SOCK_STREAM; // TCP stream sockets
	hints.ai_flags = 0;     // fill in my IP for me
	struct addrinfo *servinfo;  // will point to the results
	// printf("trying to connect to %s:%s\n", IP_addr, portnum);
	if (getaddrinfo(IP_addr, portnum, &hints, &servinfo)) die("getaddrinfo");
...
bool server_request_success(int sockfd, string& to_server_msg) {
	string response = send_msg_wait_result(sockfd, to_server_msg);
	// printf("response: %s\n", response.c_str());
	return response == "SUCCEED"; //if msg == SUCCESS, return true
}

#define DONTWAIT 0
#define NUMBER_OF_FDS 1
void print_mailbox_msgs(int sockfd) { // prints messages from the mailbox
...
void send_actual_file(int sockfd, string& username, string& filename) {
	string dirname = username + "_file_dir/" + filename; int fd, size;
	if ((fd = open(dirname.c_str(), O_RDONLY)) < 0) puts("can't open file"), exit(0);
