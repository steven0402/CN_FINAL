...
string get_name_from_list(vector<string>& list) {
	while (1) {
		printf("Choices:\n"); 
		for (int i = 0; i < list.size(); ++i) printf("%2d: %s\n", i, list[i].c_str()); puts("");
		printf("Enter your choice: "); 
		int choice; cin >> choice;
		if (cin.fail()) continue;
		if (0 <= choice && choice < list.size()) {cout << "you chose: " << list[choice] << endl; return list[choice];}
...
void save_file(int sockfd, string& filesize, string& username, string& target_file) {
	...
	printf("file size: %d\n", file_size);
	int recieved_amount = 0, n; char buf[1024] = {};
...
void dump_file(string& file) {

}

string get_msg_from_user() {
...
tuple<string, int> get_file_name(string& username) {
	...
	d = opendir(c_dirname);
	if (d) {
		for (int i = 0; (dir = readdir(d)) != NULL; i++) if (dir->d_type == DT_REG) dirlist.push_back(string(dir->d_name));
		if (!dirlist.size()) { 
	...
	else {printf("Error in opening directory!\n"); return make_tuple("", -1);}
}

void parse_userlist(string& response, vector<string> &user_list) { //unpack
	char c_response[4096], name[128]; strcpy(c_response, response.c_str());
	char *ptr = strtok(c_response, "\n");
	// ptr = strtok(NULL, "\n"); // remove the request msg
	while (ptr) {
		strcpy(name, ptr);
